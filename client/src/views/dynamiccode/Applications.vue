  ...
  <page-header-wrapper>
    ...
    <template v-slot:extraContent>
      <div class="extra-content">
        <div class="stat-item">
          <a-statistic title="生产环境启用数" :value="count" />
        </div>
        <div class="stat-item">
                ...
                <a-list-item-meta>
                  <div slot="title">
                    <a @click="handleItemClick(item.appid)">
                      {{ item.project }}
                      <span
                        style="font-size: 12px; color: rgba(0, 0, 0, 0.3)"
                        >{{ item.appid }}</span
                      >
                    </a>
                  </div>
                  <div slot="description">{{ item.desc || '暂无' }}</div>
                </a-list-item-meta>
                <div>
                  <a-popconfirm
...
export default {
  ...
  methods: {
    ...
    handleItemClick(appid) {
      this.$router.push({ path: `/dynamiccode/configuration/${appid}` })
    }
  }
}
</script>
